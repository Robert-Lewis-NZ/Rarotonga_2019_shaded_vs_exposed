...
FqFmCI <- FqFmfits %>%
  unnest(fit %>% map(~ confint2(.x, level = 0.95) %>% data.frame() %>% rename(conf.low = X2.5.., conf.high = X97.5..))) %>% 
  group_by(PAMID) %>%
...
# (shaded estimate - exposed estimate)/exposed estimate)
...
FqFm_max_summary_plot <- FqFmparams %>%
  filter(term == "FqFmmax") %>%
  ggplot() +
  geom_boxplot(aes(habitat, estimate)) +
  theme(aspect.ratio = 1, axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.1)) +
  ylab("Fq/Fm max") +
  xlab("Habitat") + 
...
Qmax_min <- dynamicQEk %>%
  group_by(PAMID) %>%
  summarise(min_EnormEk = min(EnormEk), max_EnormEk = max(EnormEk))
...
dynamQ_max_EnormEk_plot <- dynamQ_max_EnormEk %>%
  ...
  geom_point(aes(col = habitat), size = 2) +
  scale_colour_manual(labels = c("shaded", "exposed"), values = c("black", "orange")) + 
  scale_shape_manual(values = c(21, 22))
...
specslong_cryptic <- specslong %>%
  filter(habitat == "cryptic")

Specs_cryptic_CI <- group.CI(reflectance ~ wl, specslong_cryptic, ci = .95) %>%
left_join(specslong_cryptic, Specs_cryptic_CI,  by = "wl")

specslong_exposed <- specslong %>%
  filter(habitat == "exposed")

Specs_exposed_CI <- group.CI(reflectance ~ wl, specslong_exposed, ci = .95) %>%
left_join(specslong_exposed, Specs_exposed_CI,  by = "wl")

specslong_summary <- rbind(Specs_cryptic_CI, Specs_exposed_CI)

specslong_me <- specslong %>%
 group_by(habitat, wl) %>%
  mutate(sd = sd(reflectance), 
         reflectance_mean = mean(reflectance),
         reflectance_me = qnorm(.975)*(sd/sqrt(98))) %>%
  select(habitat, reflectance_mean, reflectance_me) %>%
  distinct()

specs_summary_plot <- specslong_me %>%
  group_by(wl, habitat) %>%
  mutate(absorbance_mean = 1-(reflectance_mean/100)) %>% 
  mutate(mean_absorbance_y.min = 1-((reflectance_mean - reflectance_me)/100)) %>%
  mutate(mean_absorbance_y.max = 1 - ((reflectance_mean + reflectance_me)/100)) %>% # Convert reflectance to absorbance
ggplot(aes(wl, absorbance_mean, colour = habitat)) +
  geom_line() +
  scale_fill_identity() +
  scale_x_wl_continuous() +
  ylim(values = c(0.5, 1)) +
  geom_ribbon(aes(ymin = mean_absorbance_y.min, ymax = mean_absorbance_y.max), linetype=2, alpha=0.1) +
  scale_colour_manual(labels = c("shaded", "exposed"), values = c("black", "orange")) +
  theme(aspect.ratio = 1) +
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
total_brightness <- spec_summary %>% 
  filter(variable == "B1") %>%
  rename(total_reflectance = value)
...
#              Df    Sum Sq   Mean Sq F value Pr(>F)    
# habitat       1 8.300e+08 830034344   105.3 <2e-16 ***
# Residuals   198 1.561e+09   7884697

leveneTest(total_reflectance ~ habitat, data = total_brightness)

#        Df F value   Pr(>F)   
# group   1  10.786 0.001209 **
#       198                 
...
# Kruskal-Wallis chi-squared = 74.225, df = 1, p-value < 2.2e-16
...
Fig_1 <- (FqFm_max_summary_plot + Ek_summary_plot) / 
(dynamQ_max_EnormEk_plot + specs_summary_plot) +
    plot_layout(ncol = 1, widths = c(40, 40, 40, 40)) + plot_annotation(tag_levels = 'A')

FqFm_max_dims <- get_dim(FqFm_max_summary_plot)
Ek_summary_aligned <- set_dim(Ek_summary_plot, FqFm_max_dims)
dynamQ_max_EnormEk_aligned <- set_dim(dynamQ_max_EnormEk_plot, FqFm_max_dims)
specs_summary_aligned <- set_dim(specs_summary_plot, FqFm_max_dims)

Fig_1 <- (FqFm_max_summary_plot + Ek_summary_aligned) / 
(dynamQ_max_EnormEk_aligned + specs_summary_aligned) +
plot_annotation(tag_levels = 'A')

plots_aligned <- align_patches(FqFm_max_summary_plot, Ek_summary_plot, dynamQ_max_EnormEk_plot, specs_summary_plot)

plots_aligned + plot_layout(ncol = 2)
